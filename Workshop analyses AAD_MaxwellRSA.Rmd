...
#CHANGE FILE NAME
clean_epi_data_path<-file.path(all_data_path,"SOUTH_AFRICA_EPI_clean.xlsx") 
...
if("health_facility" %in% names(epi_data) == FALSE){
  epi_data$health_facility<-epi_data$`form facility`
}
...
#residents %>% 
 # tbl_summary(include= genotypes_to_include, by = "province",missing = "no") %>% add_n() %>% 
  #modify_header(label = "Resistance Marker")%>% 
  #modify_caption("**Table 7. Prevalence of resistance genotypes and species composition by province (residents)**") %>%
  #add_ci()
...
plot_drprev<-function(genotype_name){
  ...
  p5 <- residents %>%
    filter(!is.na(!!sym(genotype_name))) %>% 
    ggplot(aes(x= health_facility, fill = factor(!!sym(genotype_name),
                                                 levels = names(plotting_colors[genotype_name][[1]])))) +
    geom_bar()+
    labs(fill = sub("_", " ",genotype_name), x= "Health Facility", y = "Count")+
    scale_fill_manual(values = unlist(plotting_colors[genotype_name][[1]]),aesthetics = c("colour", "fill"))+
    theme_classic2()+
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
          legend.title = element_blank())+
    ggtitle(paste("Count of ", sub("_", " ",genotype_name), " by health facility"))
  ...
  p7<-ggarrange(p,p2,p3,p4,p5,p6, nrow =3, ncol = 2, labels = c("a","b","c","d", "e", "f"))
  p7
  #close file
  # dev.off()
  return(list(p7))
